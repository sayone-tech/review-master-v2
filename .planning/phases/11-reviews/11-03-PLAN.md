...
must_haves:
  truths:
    ...
    - "After every bulk_create page, search_vector is populated for newly inserted/updated rows so REVW-02 keyword search returns results"
  artifacts:
    ...
    - path: apps/reviews/tests/test_sync_service.py
      provides: 9+ unit tests for SYNC requirements (incl. search_vector population)
  ...
  key_links:
    ...
    - from: apps/reviews/services/sync.py
      to: apps/reviews/models.Review.search_vector
      via: Review.objects.filter(shop=shop, search_vector__isnull=True).update(search_vector=SearchVector(...)) after bulk_create
      pattern: 'search_vector__isnull'
---
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test 9: search_vector is non-NULL for newly inserted reviews after _persist_page runs (RESEARCH.md Pitfall 3 — required for REVW-02 keyword search)
  </behavior>
  <action>
...
def _persist_page(
    *,
    ...
    Review.objects.filter(
        shop=shop, search_vector__isnull=True
    ).update(
...
def test_search_vector_populated_after_persist_page(patched_dependencies) -> None:
    ...
    This test asserts the search_vector__isnull update is wired by checking
    that newly inserted rows have a non-NULL search_vector after a page is
    persisted.
    """
    shop = _make_shop()
    page = {
    ...
    with patch.object(sync_mod, "list_reviews", return_value=page):
        sync_mod.run_initial_backfill(shop_id=shop.pk)
    # Re-fetch from DB; search_vector must be populated for both rows.
    null_count = Review.objects.filter(
        shop=shop, search_vector__isnull=True
    ).count()
    assert null_count == 0, "search_vector must be populated for all newly fetched rows"
    populated = Review.objects.filter(
        shop=shop, search_vector__isnull=False
    ).count()
    assert populated == 2
```
  </action>
  ...
  <acceptance_criteria>
    ...
    - apps/reviews/services/sync.py contains "from django.contrib.postgres.search import SearchVector"
    - apps/reviews/services/sync.py contains "search_vector__isnull=True"
    - apps/reviews/services/sync.py contains "SearchVector(\"comment\", \"reviewer_display_name\""
    ...
    - apps/reviews/tests/test_sync_service.py contains "search_vector__isnull"
    - apps/reviews/tests/test_sync_service.py contains "def test_search_vector_populated_after_persist_page"
    - Command `pytest apps/reviews/tests/test_sync_service.py -x -q` exits 0
  </acceptance_criteria>
  <done>sync.py module implements all 9 SYNC requirements + RESEARCH.md Pitfall 3 (search_vector population); 9 tests pass.</done>
</task>

</tasks>

<verification>
- `pytest apps/reviews/tests/test_sync_service.py apps/reviews/tests/test_progress_service.py -x -q` passes (18 tests total)
...
- search_vector is populated after every bulk_create (RESEARCH.md Pitfall 3) so REVW-02 keyword search works for newly fetched reviews
- All tests use mocks — no real Google API or Redis required
</success_criteria>

<output>
After completion, create `.planning/phases/11-reviews/11-03-SUMMARY.md` documenting: service function signatures, AuditLog event vocabulary used, Redis key conventions, search_vector update strategy (per-page after bulk_create per RESEARCH.md Pitfall 3), and any deviations from RESEARCH.md Patterns 4-5.
</output>
