...
- **Rate limit (QUEUE-02):** global Celery `rate_limit` on the enrichment task, value from an env-configurable setting, default ~500/min; applies per task-type across all workers.
...
- `docs/ReviewBee_Canonical_Tag_Requirements_v1.0.docx` — the source spec (v1.0, Final). §3 solution principles, §4 data model (NOTE: written against the old JSONB model — superseded by the relational reconciliation below), §6 AI pipeline changes, §6.4 token cost. **Read with the reconciliation — do not take §4's JSONB/`canonical_tag_id` claims literally.**
