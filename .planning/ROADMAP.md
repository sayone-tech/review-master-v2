...
- [ ] 01-06-PLAN.md — Gap closure: fix mobile sidebar Alpine stale-copy bug + add vite_react_refresh so React widgets mount on /__ui__/
...
- [ ] 03-06-PLAN.md — Gap closure: sidebar nav href, DataTable sticky-thead, Create-modal useCallback, list.html mount gating, pagination per-page extraction
...
**Success Criteria** (what must be TRUE):
  ...
  4. `manage.py check --deploy` passes against `config.settings.production` with required secrets stubbed
**Plans**: 4 plans
...
| 5. Profile and Hardening | 0/4 | Planned | - |
