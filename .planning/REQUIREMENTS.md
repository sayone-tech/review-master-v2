...
- [x] **ACTN-02**: Action Items list page at `/admin/org/action-items/`; Staff queryset filtered to `scope = SHOP AND shop_id IN (staff's accessible shops)`; brand-scoped items hidden from Staff at API layer (403 on direct access)
- [x] **ACTN-03**: Filter bar: Store, Status (To Do / In Progress / Complete / Won't Do), Scope toggle (Shop / Brand / All — Org Admin / Manager only; hidden from Staff), Assignee (All / Assigned to me / Unassigned / by name), From Date, To Date, Search (title + notes)
...
- [x] **ACTN-05**: Pagination with 10/25/50/100 selector (default 25); sort: Newest first (default), Oldest first, Due Date ascending, Status (To Do first), Priority (high first)
- [x] **ACTN-06**: Action Item modal with three tabs: Details (read-only + Edit button), Notes (append-only timeline), Source Review (visible only for AI-extracted items); modal title shows action item title
- [x] **ACTN-07**: Edit mode: Title, Priority, Due Date, Assignee are editable; Scope and Shop are NOT editable for AI-extracted items; Status changes only via dedicated status buttons or row actions menu
...
- [x] **ACTN-09**: Manual action item creation via "+ New Action Item": fields Title (5–200 chars), Scope (Shop/Brand), Shop (required when Scope=Shop), Priority, Assignee, Due date (today or future), Initial note; Brand scope option hidden from Staff
- [x] **ACTN-10**: Notes are append-only (1–2000 chars each); existing notes cannot be edited or deleted
- [x] **ACTN-11**: Source Review tab renders read-only review card; "Open in Reviews" link navigates to `/admin/org/reviews/?id={review_id}`
- [x] **ACTN-12**: `GET /api/v1/action-items/` resolves in ≤5 SQL queries; verified by `CaptureQueriesContext` test in CI
...
| ACTN-02 | Phase 13 | Complete |
| ACTN-03 | Phase 13 | Complete |
| ACTN-04 | Phase 13 | Complete |
| ACTN-05 | Phase 13 | Complete |
| ACTN-06 | Phase 13 | Complete |
| ACTN-07 | Phase 13 | Complete |
| ACTN-08 | Phase 13 | Complete |
| ACTN-09 | Phase 13 | Complete |
| ACTN-10 | Phase 13 | Complete |
| ACTN-11 | Phase 13 | Complete |
| ACTN-12 | Phase 13 | Complete |
